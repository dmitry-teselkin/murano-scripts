...
if [ $distro_based_on == "redhat" ]; then
    ...
    rsync_apache_logs="/var/log/httpd/error_log"
else
    STORE_AS_ARTIFACTS+=" /var/log/apache2/error.log"
    rsync_apache_logs="/var/log/apache2/error_log"
fi

RSYNC_BUILD_ARTIFACTS="
--recursive /tmp/murano-artifacts/
--recursive ${PROJECT_DIR}/${TESTS_DIR}/screenshots/
...
function start_xvfb_session() {
    ...
    #$SCREEN_CMD -dmS display sudo Xvfb -fp ${fonts_path} :${VFB_DISPLAY_NUM} -screen 0 ${VFB_DISPLAY_SIZE}x${VFB_COLOR_DEPTH}
